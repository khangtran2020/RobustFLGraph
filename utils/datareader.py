...
class DataReader():
    ...
    def __init__(self, args):
        ...
        # split for number of clients
        idx = self.rnd_state.permutation(N_graphs)
        num_graph_per_client = int(N_graphs / args.num_client)
        compromise_client = self.rnd_state.choice(range(args.num_client), args.num_compromised_client, replace=False)
        for i in range(args.num_client):
            data['client_{}'.format(i)] = {}
            data['client_{}'.format(i)]['gid'] = idx[i * num_graph_per_client: (i + 1)*num_graph_per_client]
            data['client_{}'.format(i)]['train'] = list(
                self.rnd_state.choice(data['client_{}'.format(i)]['gid'], int(args.train_ratio * num_graph_per_client),
                                      replace=False))
            data['client_{}'.format(i)]['test'] = list(
                set(data['client_{}'.format(i)]['gid']) - set(data['client_{}'.format(i)]['train']))
            if i in compromise_client:
                data['client_{}'.format(i)]['compromised'] = True
            else:
                data['client_{}'.format(i)]['compromised'] = False
